...
)
{
	...
	_tcprintf(_T("%s"), _T("hello"));

    CApiOverride* pApiOverride;
	g_cLogger.InitAll(L"C:\\hello.txt");
	g_cLogger.WriteLog(L"=============start===============");
    ...
    // start all
    if (!pApiOverride->Start(_T("C:\\Program Files (x86)\\Notepad++\\notepad++.exe"),// app path
...
void STDMETHODCALLTYPE CallBackBeforeAppResume(DWORD dwProcessID,PVOID pUserParam)
{
    ...
    pApiOverride->LogOnlyBaseModule(TRUE); // log all module not only the .exe
    //->SetModuleFilteringWay(FILTERING_WAY_ONLY_SPECIFIED_MODULES); // log only specified modules
    //pApiOverride->AddToFiltersModuleList(_T("C:\\Users\\VietHQ\\Desktop\\winapioverride32_bin (1)\\monitoring files\\HookedOnlyModuleList.txt")); // set filters by file containing multiple definitions
    ...
    //pApiOverride->LoadMonitoringFile(_T("C:\\Users\\VietHQ\\Desktop\\winapioverride32_src (1)\\monitoring files\\InputTextDataRetrival.txt"));
	pApiOverride->LoadMonitoringFile(_T("C:\\monitoring files\\kernel32.txt"));
	pApiOverride->LoadMonitoringFile(_T("C:\\monitoring files\\wininet.txt"));
	pApiOverride->LoadMonitoringFile(_T("C:\\monitoring files\\User32.txt"));
...
void STDMETHODCALLTYPE CallbackMonitoringLog(LOG_ENTRY* pLog,PVOID pUserParam)
{
	...
	CString tmp = "";
	tmp.Append(pLog->pszModuleName);
	tmp.Append(L"!");
	tmp.Append(pLog->pszApiName);
	tmp.Append(L"(");
	for (int i = 0; i < pLog->pHookInfos->bNumberOfParameters; i++)
	{
		tmp.Append(pLog->ParametersInfoArray[i].pszParameterName);
		if (i < pLog->pHookInfos->bNumberOfParameters - 1)
		{
			tmp.Append(L",");
		}
	}
	...
	for (int i = 0; i < pLog->pHookInfos->bNumberOfParameters; i++)
	{
		...
		if (pLog->ParametersInfoArray[i].dwType == tagSupportedParameters::PARAM_DWORD ||
			pLog->ParametersInfoArray[i].dwType == tagSupportedParameters::PARAM_HANDLE)
		{
			strNumber.Format(L"0x%X", pLog->ParametersInfoArray[i].Value);
		}
		else if (pLog->ParametersInfoArray[i].dwType == tagSupportedParameters::PARAM_PWSTR)
		{
			WCHAR *a = new WCHAR[pLog->ParametersInfoArray[i].dwSizeOfPointedValue + 10];

			int b = 0;
		}

		tmp.Append(strNumber);
	...
	g_cLogger.WriteLog(tmp.GetBuffer());
	//printf("%s - %s", pLog->pszApiName, pLog->pszModuleName);
