...
int main()
{
    ...
    if (hModule != nullptr)
    {
        ...
        else
        {
			...
			// start all
			if (!pApiOverride->Start(L"C:\\Program Files (x86)\\Notepad++\\notepad++.exe",// app path
...
void STDMETHODCALLTYPE CallBackBeforeAppResume(DWORD dwProcessID, PVOID pUserParam)
{
	...
	pApiOverride->LoadMonitoringFile(L"kernel32.txt");
	pApiOverride->LoadMonitoringFile(L"C:\\monitoring files\\wininet.txt");
	pApiOverride->LoadMonitoringFile(L"C:\\monitoring files\\User32.txt");
